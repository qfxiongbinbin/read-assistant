import { containsCJK, hardRatio, isKnownWord, parseResult, properNouns, verify, words } from '../src/lib/verify';
...
check('plain json', parseResult('{"simplified":"Hello.","glossary":[]}')?.simplified === 'Hello.');
check(
  'fenced json + glossary',
  parseResult('Here you go:\n\`\`\`json\n{"simplified":"Hi.","glossary":[{"term":"a","simple":"b"}]}\n\`\`\`')?.glossary.length === 1,
);
check('garbage -> null', parseResult('no json at all') === null);
check('empty simplified -> null', parseResult('{"simplified":"  ","glossary":[]}') === null);
...
const good = 'The group started a full check. They wanted to know what caused the sudden end of the project. The project was expected to finish in 2024.';
const goodOutcome = verify(original, { simplified: good, glossary: [] });
...
const cjk = verify(original, { simplified: '\u8fd9\u662f\u4e00\u4e2a\u7b80\u5355\u7684\u7248\u672c\u3002', glossary: [] });
...
check('rejects long sentence', !longSentence.ok && longSentence.violations.some((v) => v.includes('longest sentence')));
...
check('rejects dropped numbers', !missing.ok && missing.violations.some((v) => v.includes('4821')));

const hard = verify('The team met.', {
  simplified: 'The ubiquitous ephemeral perspicacious obfuscated defenestrated recalcitrant team convened ostensibly.',
  glossary: [],
});
check('rejects hard words', !hard.ok && hard.violations.some((v) => v.includes('uncommon words')));
