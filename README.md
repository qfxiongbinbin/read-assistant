...
|---|---|
| 点击段落 | 整个段落 |
| **选中一句话**（或任意一段文字） | 只处理选中的部分 |

选中文字后，选区上方会出现一个蓝色 **Simplify** 胶囊按钮，点它即可。选区太短（不足 20 字符且不足 12 个词）不会出现按钮。

两种触发得到的结果结构完全一样。

## 结果长什么样

面板里固定三段，全部是英文，**不会出现中文**：

| 区块 | 内容 |
|---|---|
| **Simplified** | 用简单句、简单词重写的版本。一个长句会被拆成 2~4 个短句 |
...
4. **必须抽取**：`keyWords` 为空会判废重试；抽取项在词频校验里被豁免，因为它们本来就是"要学的词"
