...
> Hover a paragraph and use the wand, select a sentence to simplify it, or select a word to explain it.
...
| 触发 | 动作 | 作用范围 |
|---|---|---|
| 段落右上角的 `✦` 按钮 | Simplify | 整个段落 |
| **选中一个词 / 一个短语 / 一句话** | Simplify 或 Explain | 只处理选中的部分 |
| **选中一整段** | Simplify | 只处理选中的部分 |

**Simplify** 把内容改写成 CEFR 指定难度的简单英文，并抽出关键单词与短语。
**Explain** 把内容用最简单的外语词重说一遍：`plainWords`（难词 → 简单词）+ `plainSentence`（一到三句简单句）。

> "Explain" 是**用更简单的外语重说**，不是译成中文——面板里始终只有英文。

划词后，选区上方会出现蓝色胶囊按钮，**按选区能不能装得下决定给哪些动作**：

| 选区 | 胶囊 | 面板 tab |
|---|---|---|
| 词 / 短语（不足 20 字符且不足 12 词） | `Explain` | Simplify 置灰：需要 20 字符或 12 词 |
| 短语 / 一句话（≥ 20 字符或 ≥ 12 词，≤ 40 词） | `Simplify` + `Explain` | 两个都能用，随时切 |
| 一整段（> 40 词） | `Simplify` | Explain 置灰：超出重说的长度上限 |

**40 词是有来历的**：Explain 的输出预算是 1~3 句、每句 ≤14 词 ≈ 42 词。超过这个预算，模型只能压缩，而**校验器看不出信息被丢了**（它只查 CJK、难词、句长、数字）。所以上限做成结构性约束，而不是指望 prompt 自律。

> 想要"整段用简单词概括"的话，那是**摘要**，不是翻译——本项目会把它做成独立动作，而不是放宽 Explain 的保真要求。

面板顶部可在 Simplify 与 Explain 间切换；不可用的动作会置灰并说明原因。
...
- 原文保留不动，简化只是对照层
- **点任意词可以一路往下钻**（见下），背面还带着音标和朗读；Explain 的正文和单词同样可点
- 弹窗位置三选一：段落底部 / 段落侧边 / 跟随光标（见下）
- 结果按段落缓存 30 天，同一段二次点击不再请求 API（Simplify 与 Explain 各自独立缓存）
- 难度三档：A2 / B1 / B2（只影响 Simplify，Explain 恒为"最简单"）
- popup：日常设置只展示难度、口音与站点开关；触发方式、音标、模型和 API Key 收入 Advanced settings
- 默认不再劫持正文点击；如需要旧行为，可在 Advanced settings 打开“Click paragraph body to simplify”
...
popup 里的 **Explain words as soon as I select them**：

| 状态 | 行为 |
|---|---|
| 关（默认） | 划词后出现胶囊，点 `Explain` 才发请求 |
...
3. 打开任意英文网页：悬停段落并点 `✦`，或选中句子点 **Simplify**，或选中单词点 **Explain**
...
| `npm run selftest` | 校验器自测（含划词解释的解析与校验），不需要 API Key |
...
1. **零中文**：注入页面的内容全部是英文；`verify.ts` 检测到 CJK 字符直接判废并触发重试。Explain 也是**用更简单的外语重说**，不是译成中文
2. **原文永不替换**：简化版是一层对照，不是替换
3. **只处理你点选的内容**：不整页改写，控制成本也避免打扰阅读
4. **必须抽取**：Simplify 的 `keyWords` 为空会判废重试；Explain 的输入含难词时 `plainWords` 不能为空
...
- 下钻解释按「单词 + 当前上下文」缓存，同一单词在不同语境中会分别生成并复用结果
- Explain 的适用范围是**一个词 ~ 一句话（≤ 40 词）**，整段会退化成有损概括，所以直接不给这个动作（`TRANSLATE_MAX_WORDS`）
- Explain 不按 CEFR 档位分级，恒定输出"最简单的词"
- 三个分界都是硬编码的（Simplify 的 20 字符 / 12 词、Explain 的 40 词 / 600 字符），不区分语言与内容
