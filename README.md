...
|---|---|---|
| 点击段落 | Simplify | 整个段落 |
| **选中一句话 / 一段话** | Simplify 或 Translate | 只处理选中的部分 |
| **选中一个词 / 一个短语** | Translate | 只处理选中的部分 |
...
划词后，选区上方会出现蓝色胶囊按钮，按选区大小给出可用动作：

- 选中的是**词或短语**（不足 20 字符且不足 12 词）→ 只出现 `Translate`，因为一个词没有可改写的句子
- 选中的是**句子或段落** → 同时出现 `Simplify` 和 `Translate`，点哪个用哪个

面板顶部有两个 tab，同一个选区可以随时切换动作，不必重新划词；词/短语选区下 `Simplify` tab 置灰。
...
- Translate 的正文限制在 1~3 句，选整段翻译时只保留主要意思，会丢细节（`MAX_TRANSLATE_CHARS` 1200 字符是后端兜底）
- Translate 不按 CEFR 档位分级，恒定输出"最简单的词"
- 长短选区的分界是硬编码的 20 字符 / 12 词，不区分语言与内容
